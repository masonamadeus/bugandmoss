  ...
  exportDeck: async function(deckId) {
    try {
      ...
      for (const card of deck.cards) {
        if (card.questionMediaId) {
          const blob = await db.getMedia(card.questionMediaId);
          if (blob) assets.file(card.questionMediaId, blob);
        }
        if (card.answerMediaId) {
          const blob = await db.getMedia(card.answerMediaId);
          if (blob) assets.file(card.answerMediaId, blob);
        }
      }
